...
новые инстансы EC2 не обладают публичным  IP адресом по умолчанию!! -Изменить параметры автоматического назначения IP адресов конфигурации подсети.

![image](https://user-images.githubusercontent.com/58170246/125336152-1b524080-e356-11eb-8e2b-e01c54812ad0.png)

![image](https://user-images.githubusercontent.com/58170246/125336209-2c02b680-e356-11eb-9b00-e3d3aa1d3015.png)

И каждый  инстансданой подсетибудет обладать публичным IP. после создания шлюза NAT инстансы расположенные в приватных подсетях не  могут загружать пакеты и обновления програмного обеспечения. Необходимо добавить маршрут от 0.0.0.0/0 к шлюзу NAT

![image](https://user-images.githubusercontent.com/58170246/125336755-da0e6080-e356-11eb-9bfc-15005e723378.png)

![image](https://user-images.githubusercontent.com/58170246/125336806-e98da980-e356-11eb-9e3a-fd3b26ae13df.png)

Трафик не поступает в инстансы. Группы безопасности имеют соответствующие типу трафика правила- проверить NICL-список контроля сетьевого доступа. 

![image](https://user-images.githubusercontent.com/58170246/125337364-95cf9000-e357-11eb-939e-9e7418b391e6.png)

ошибка при подключении нескольких интернет шлюзов к VPC или ошибкаприподключении нескольких приватных виртуальных шлюзов. 

К однной VPS может быть подключен только один интернет шлюз. и только один VPG

Группа безопасности VPC имеет недостаточно правил для работы приложения. -назначить инстенсу дополнительные группы безопасности. Создать определенный набор групп безопасности, который соответствует нуждам приложения. 

Невозможно установить соединение через SSH внепри приватной подсети. Я не установила VPN, или не установила соединение с инстансом через бастианхост (должен присутствовать в VPC для соединения с приватной подсетью), альтернатива VPN.

Соединениесайт-сайт успешно установлено через VPN но отсутствует доступ к расширеннным ресурсам.  -решение добавить необходимое правило в таблицу маршрутизации VPG. 

Не возможно установить взаимные соединения билинг конекшен между двумя VPC, расположеними в различных регионах-соединение может быть только между  VPC в одном регионе. 

7 – Мои проблемы, которые возникали с ELB

Инстансы работают нормально но ELB не можжет подтвердить их работоспособность. проверить конфигурацию провери работоспособности 
